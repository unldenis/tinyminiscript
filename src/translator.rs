use crate::parser::{AST, ASTVisitor, Fragment};
...
impl ASTVisitor<String> for TranslatorVisitor {
    ...
    fn visit_ast(&mut self, node: &AST) -> Result<String, Self::Error> {
        match &node.fragment {
            ...
            Fragment::AndV { x, y } => Ok(format!("{} {}", self.visit_ast(x)?, self.visit_ast(y)?)),
            ...
            Fragment::Thresh { k, xs } => {
                ...
                for x in xs {
                    script.push_str(&self.visit_ast(x)?);
                    script.push_str(" ADD ");
                }
            ...
            Fragment::Identity { identity_type, x } => match identity_type {
                crate::parser::IdentityType::A => {
                    Ok(format!("TOALTSTACK {} FROMALTSTACK", self.visit_ast(x)?))
                }
                crate::parser::IdentityType::S => Ok(format!("SWAP {}", self.visit_ast(x)?)),
                crate::parser::IdentityType::C => Ok(format!("{} CHECKSIG", self.visit_ast(x)?)),
                crate::parser::IdentityType::D => {
                    Ok(format!("DUP IF {} ENDIF", self.visit_ast(x)?))
                }
                crate::parser::IdentityType::V => Ok(format!("{} VERIFY", self.visit_ast(x)?)),
                crate::parser::IdentityType::J => {
                    Ok(format!("SIZE 0NOTEQUAL IF {} ENDIF", self.visit_ast(x)?))
                }
                crate::parser::IdentityType::N => Ok(format!("{} 0NOTEQUAL", self.visit_ast(x)?)),
            },
        }
    }
}
...
mod tests {
    ...
    fn test_translator_visitor() {
        ...
        let result = visitor.visit_ast(&ast).unwrap();
